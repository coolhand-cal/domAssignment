...
function render(pool) {
 ...
 const cont = pool.map(renderContact);

 

  contactSection.innerHTML = cont;
}
...
function filterByCity(city) {
  ...
  const pool=window.contacts;

  const result = pool.filter((contact) => contact.address.city.toLowerCase() === city);
...
function filterHandler(event) {
  const list = document.querySelector(`#filterOptions`);
 event.preventDefault();
    const selected = list.value;
    const newPool = filterByCity(selected);
...
function loadCities(contacts) {
  ...
  const cities = contacts.map((contact) => {
    const thisCity = contact.address.city.toLowerCase();
    const missing = (!allcitites.includes(thisCity));
 
    if (missing) {
      allcitites.push(thisCity);
      return `<option value="${thisCity}">${thisCity}</option>`
    }
  
  });
  
  options.innerHTML += cities;
 // return options;
...
function main() {
  ...
  list.addEventListener("change", filterHandler);
  loadCities(pool);
  render(pool);
